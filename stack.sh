#!/usr/bin/env bash
# =============================================================================
# stack.sh — lifecycle wrapper for the Solace DT Observatory
#
# Exists because every component in this pipeline can report itself healthy
# while doing nothing useful. `stack.sh verify` turns the source readme's
# Stage 10 into automated checks so that failure mode surfaces immediately.
#
# It also owns the mapping from BROKER_MODE to COMPOSE_PROFILES, so the broker
# switch stays a single variable rather than two that must agree.
#
...
# Compose invocation, assembled from .env so the two can never disagree:
#   - the local-broker profile is enabled only in local mode
#   - the JSONL override is layered in only when the file sink is on
compose() {
  local files=(-f docker-compose.yaml)
  if [ "${TRACE_FILE_ENABLED:-false}" = "true" ]; then
    files+=(-f docker-compose.jsonl.yaml)
  fi
  local profiles=""
  if [ "$BROKER_MODE" = "local" ]; then
    profiles="local-broker"
  fi
  COMPOSE_PROFILES="$profiles" docker compose "${files[@]}" "$@"
}
...
cmd_up() {
  load_env up
  info "Bringing up the stack (BROKER_MODE=$BROKER_MODE, TRACE_FILE_ENABLED=${TRACE_FILE_ENABLED:-false})"
  if [ "$BROKER_MODE" = "external" ]; then
    info "External broker: $SOLACE_BROKER_HOST:$SOLACE_BROKER_AMQP_PORT — no broker container will be started"
  fi
  compose up -d
  printf '\n'
  info "Started. The collector waits for broker-bootstrap to finish, so give it a few seconds."
  printf '\n'
  cmd_urls
  printf '\n'
  printf 'Send some traffic, then run: %s./stack.sh verify%s\n' "$BOLD" "$RST"
}

cmd_down() {
  load_env down
  info "Stopping the stack — volumes are kept, so all configuration and traces survive"
  compose down
...
# Re-run broker configuration on demand. Idempotent, so this is always safe —
# use it after editing telemetry settings in .env, or to configure a broker
# that was not reachable at `up` time.
cmd_setup() {
  load_env setup
...
# The only destructive command. Named explicitly, confirms explicitly.
cmd_reset() {
  load_env reset
  printf '%sThis deletes all volumes:%s\n' "$BOLD$RED" "$RST"
  printf '  - broker configuration and message spool\n'
  printf '  - every trace stored in Tempo\n'
...
cmd_urls() {
  ...
  printf '  Tempo API          http://localhost:%s\n' "${PORT_TEMPO}"
  printf '  Collector health   http://localhost:%s\n' "${PORT_OTEL_HEALTH}"
  printf '  Collector zpages   http://localhost:%s/debug/tracez\n' "${PORT_OTEL_ZPAGES}"
  if [ "$BROKER_MODE" = "local" ]; then
    ...
    printf '                  -ptl=%s/demo -mn=100 -mr=10\n' "${SOLACE_MSG_VPN}"
  else
...
usage() {
  cat <<EOF
${BOLD}stack.sh${RST} — Solace Distributed Tracing Observatory
  ...
  ${BOLD}verify${RST}      Check every hop of the pipeline and report what is broken.
              --persistence  additionally restart and confirm state survives.
  ${BOLD}logs${RST} [svc]  Follow logs. Service names: solbroker, otel-collector,
              tempo, grafana, broker-bootstrap.
...
Typical first run:
  ...
  ./stack.sh up
  # ... send traffic with sdkperf or your own client ...
  ./stack.sh verify
EOF
}
